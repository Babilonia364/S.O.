...
void memDezDez(pid_t pid, sem_t* sync1, sem_t* sync2, int *s, int c, int *mem, int i)
{
	int j=0, turn=0;
	...
	if(i<10)
	{
		/* Processos filho que serão processos consumidores */
		while(j<10)
		{
			if(turn==0)
			{
	...
	{
		/* Processos filhos que serao processos produtores */
		while(i)
		{
			if(turn==0)
			{
